...
interface AddAnimalToPublicEventFormProps {
  curPublicEvent: PublicEvent;
}

function AddAnimalToPublicEventForm(props: AddAnimalToPublicEventFormProps) {
  ...
  const [animalTargetList, setAnimalTargetList] = useState<Animal[]>(
    curPublicEvent.animals || []
  );
  const targetAnimalCodesSet = curPublicEvent.animals
    ? new Set(curPublicEvent.animals.map((animal) => animal.animalCode))
    : new Set();
  ...
  function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const assignAnimalsToPublicEventApi = async () => {
      try {
        ...
        // success
        toastShadcn({
          description: "Successfully added animal(s) to animal activity",
        });
        navigate(-1);
      } catch (error: any) {
        // got error
...
export default AddAnimalToPublicEventForm;