...
function ViewEmployeeRoleDetails(props: EmployeeInfoDetailsProps) {
  ...
  const [updateSpecializationDialog, setUpdateSpecializationDialog] =
  ...
  const disableAccountManager = async () => {
    try {
      const responseJson = await apiJson.put(
        `http://localhost:3000/api/employee/unSetAccountManager/${curEmployee.employeeId}`,
        curEmployee
      );
      ...
      toastShadcn({
        ...
        description: "Successfully revoke access: " + curEmployee.employeeName,
      });
  ...
  const enableAccountManager = async () => {
    try {
      const responseJson = await apiJson.put(
        `http://localhost:3000/api/employee/setAccountManager/${curEmployee.employeeId}`,
        curEmployee
      );
      ...
      toastShadcn({
        ...
        description:
          "Successfully granted Account Manager access: " +
  ...
  const disableRole = async () => {
    try {
      ...
      const responseJson = await apiJson.put(
        `http://localhost:3000/api/employee/getEmployee/${curEmployee.employeeId}/disableRole`,
        roleJson
      );
      ...
      toastShadcn({
        ...
        title: "Uh oh! Something went wrong.",
        description:
          "An error has occurred while disabling " +
          role +
          " access: \n" +
          apiJson.error,
      });
  ...
  const disableRoleDialogFooter = (
    <React.Fragment>
      ...
      {role && (
        <Button variant={"destructive"} onClick={disableRole}>
          <HiCheck />
          Yes
        </Button>
      )}
  ...
  async function enableRole(e: any) {
    try {
      ...
      const responseJson = await apiJson.put(
        `http://localhost:3000/api/employee/getEmployee/${curEmployee.employeeId}/enableRole`,
        result
      );
  ...
  async function updateSpecialization(e: any) {
    try {
      ...
      const responseJson = await apiJson.put(
        `http://localhost:3000/api/employee/getEmployee/${curEmployee.employeeId}/updateSpecializationType`,
        result
      );
  ...
  async function updateRole(e: any) {
    try {
      ...
      const responseJson = await apiJson.put(
        `http://localhost:3000/api/employee/getEmployee/${curEmployee.employeeId}/updateRoleType`,
        result
      );
      ...
      toastShadcn({
        // variant: "destructive",
        title: "Role Type is updated!",
        description: "Successfully update role type" + curEmployee.employeeName,
      });
      //setRefreshSeed(refreshSeed + 1);
    } catch (error: any) {
      // got error
  ...
  const enableRoleDialogFooter = (
    <React.Fragment>
      ...
      {role && (
        <Button variant={"destructive"} onClick={() => enableRole}>
          <HiCheck />
          Yes
        </Button>
      )}
    ...
    <div>
          ...
          <TableBody>
            <TableRow>
              ...
              <TableCell className="w-1/6" colSpan={1}>
                {curEmployee.isAccountManager ? "Yes" : "No"}
              </TableCell>
              {!curEmployee.dateOfResignation && (
                <TableCell className="w-1/6" colSpan={2}>
                  {curEmployee.isAccountManager ? (
                    <Button type="button" onClick={disableAccountManager}>
                      Revoke access
                    </Button>
                  ) : (
                    <Button type="button" onClick={enableAccountManager}>
            ...
            <TableRow>
              {curEmployee.keeper && (
                <TableCell className="w-1/6 font-bold">Disabled</TableCell>
              )}
              {curEmployee.keeper && (
                <TableCell>
                  {curEmployee.keeper.isDisabled ? "Yes" : "No"}
                </TableCell>
              )}
              {!curEmployee.dateOfResignation && (
                <TableCell className="w-1/3" rowSpan={1} colSpan={1}>
                  {!curEmployee.keeper || curEmployee.keeper.isDisabled ? (
                    <Button
                      type="button"
                      onClick={() => confirmEnableRole("Keeper")}
                    >
                      Enable
                    </Button>
                  ) : (
                    <Button
                      type="button"
                      onClick={() => confirmDisableRole("Keeper")}
                    >
                      Disable
                    </Button>
                  )}
            ...
            {curEmployee.keeper && (
              <TableRow>
                ...
                <TableCell colSpan={1}>
                  {beautifyText(curEmployee.keeper.keeperType)}
                </TableCell>
                {!curEmployee.keeper.isDisabled && (
                  <TableCell colSpan={1}>
                    <Button
                      type="button"
                      onClick={() => updateRoleType("Keeper")}
                    >
                      Change
                    </Button>
                  </TableCell>
            ...
            {curEmployee.keeper && (
              <TableRow>
                ...
                <TableCell colSpan={1}>
                  {beautifyText(curEmployee.keeper.specialization)}
                </TableCell>
                {!curEmployee.keeper.isDisabled && (
                  <TableCell colSpan={1}>
                    <Button
                      type="button"
                      onClick={() => updateSpecializationType("Keeper")}
                    >
                      Change
                    </Button>
                  </TableCell>
            ...
            <TableRow>
              ...
              {curEmployee.generalStaff && (
                <TableCell colSpan={1}>
                  {curEmployee.generalStaff.isDisabled ? "Yes" : "No"}
                </TableCell>
              )}
              ...
              <TableCell className="w-1/3" rowSpan={1} colSpan={1}>
                ...
                curEmployee.generalStaff.isDisabled ? (
                  <Button
                    type="button"
                    onClick={() => confirmEnableRole("General Staff")}
                  >
                    Enable
                  </Button>
                ) : (
                  <Button
                    type="button"
                    onClick={() => confirmDisableRole("General Staff")}
                  >
                    Disable
                  </Button>
                )}
            ...
            {curEmployee.generalStaff && (
              <TableRow>
                ...
                <TableCell colSpan={1}>
                  {beautifyText(curEmployee.generalStaff.generalStaffType)}
                </TableCell>
                {!curEmployee.generalStaff.isDisabled && (
                  <TableCell colSpan={1}>
                    <Button
                      type="button"
                      onClick={() => updateRoleType("General Staff")}
                    >
                      Change
                    </Button>
                  </TableCell>
            ...
            <TableRow>
              ...
              {curEmployee.planningStaff && (
                <TableCell colSpan={1}>
                  {curEmployee.planningStaff.isDisabled ? "Yes" : "No"}
                </TableCell>
              )}
              <TableCell className="w-1/3" rowSpan={1} colSpan={1}>
                {!curEmployee.planningStaff ||
                curEmployee.planningStaff.isDisabled ? (
                  <Button
                    type="button"
                    onClick={() => confirmEnableRole("Planning Staff")}
                  >
                    Enable
                  </Button>
                ) : (
                  <Button
                    type="button"
                    onClick={() => confirmDisableRole("Planning Staff")}
                  >
                    Disable
                  </Button>
                )}
            ...
            {curEmployee.planningStaff && (
              <TableRow>
                ...
                <TableCell colSpan={1}>
                  {beautifyText(curEmployee.planningStaff.plannerType)}
                </TableCell>
                {!curEmployee.planningStaff.isDisabled && (
                  <TableCell colSpan={1}>
                    <Button
                      type="button"
                      onClick={() => updateRoleType("Planning Staff")}
                    >
                      Change
                    </Button>
                  </TableCell>
            ...
            {curEmployee.planningStaff && (
              <TableRow>
                ...
                <TableCell colSpan={1}>
                  {beautifyText(curEmployee.planningStaff.specialization)}
                </TableCell>
                {!curEmployee.planningStaff.isDisabled && (
                  <TableCell colSpan={1}>
                    <Button
                      type="button"
                      onClick={() => updateSpecializationType("Planning Staff")}
                    >
                      Change
                    </Button>
                  </TableCell>
      ...
      >
        <div className="confirmation-content">
          ...
          {curEmployee && (
            <span>
              Are you sure you want to disable <b>{curEmployee.employeeName}</b>
              ?
            </span>
          )}
        </div>
        ...
        >
          ...
          <Form.Submit asChild>
            <>
              ...
              <button
                className="mt-10 h-12 w-2/3 self-center rounded-full border bg-primary text-lg text-whiten transition-all hover:bg-opacity-80"
                onClick={hideSpecializationTypeDialog}
              >
                Cancel
              </button>
            </>
