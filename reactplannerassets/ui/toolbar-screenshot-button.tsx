...
// import ReactPlannerContext from "../../../src/react-planner-context";
import ReactPlannerContext from "../../reactplanner-src/react-planner-context";
import {
...
export default function ToolbarScreenshotButton({ mode }) {
  let { translator }: any = useContext(ReactPlannerContext);

  let imageBrowserDownload = (imageUri) => {
    let fileOutputLink = document.createElement("a");

    let filename: any = "output" + Date.now() + ".png";
  ...
  let saveSVGScreenshotToFile = (event) => {
    ...
    let heightAtt: any = document.createAttribute("height");
