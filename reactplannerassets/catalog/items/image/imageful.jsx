...
import { ReactPlannerContext } from "../../../../reactplanner-src/index";
...
const ImageFul = (props) => {
  ...
  const onMouseMove = (event) => {
    ...
    if (state.handleMouseMove1) {
      ...
      projectActions.setProperties(
        new Map({ x1: newX, y1: newY, distance: new Map({ length: dist }) })
      );
    } else if (state.handleMouseMove2) {
      let dist = pointsDistance(x1, y1, newX, newY);
      projectActions.setProperties(
        new Map({ x2: newX, y2: newY, distance: new Map({ length: dist }) })
      );
    }
  };
  ...
  useEffect(() => {
    ...
    if (imageUri) {
      ...
      img.onload = () => {
        ...
        projectActions.setProperties(
          new Map({ width: img.naturalWidth, height: img.naturalHeight })
        );
      };
      img.onerror = () => {
  ...
  let ruler = !element.selected ? null : (
    <g>
      ...
      <circle
        ...
        cy={y1}
        r={
          state.hover1 || state.handleMouseMove1
            ? hoverCircleRadius
            : grabCircleRadius
        }
        style={
      ...
      <circle
        ...
        cy={y2}
        r={
          state.hover2 || state.handleMouseMove2
            ? hoverCircleRadius
            : grabCircleRadius
        }
        style={
  ...
  return (
    ...
    >
      {imageUri && !state.imageLoadError ? (
        <image xlinkHref={imageUri} x="0" y="0" width={width} height={height} />
      ) : (
        <g>
